-- Customer app inquiry attachments:
-- 1) Allow anon uploads into inquiry-images/customer-app/*
-- 2) Accept image_url + additional_image_urls on submit_customer_inquiry
...
drop policy if exists "customer_app_inquiry_images_insert" on storage.objects;
create policy "customer_app_inquiry_images_insert"
on storage.objects
for insert
to anon, authenticated
with check (
  bucket_id = 'inquiry-images'
  and (storage.foldername(name))[1] = 'customer-app'
);

drop policy if exists "customer_app_inquiry_images_select" on storage.objects;
create policy "customer_app_inquiry_images_select"
on storage.objects
for select
to public
using (bucket_id = 'inquiry-images');

drop function if exists public.submit_customer_inquiry(text, text, text, text, text, text);
drop function if exists public.submit_customer_inquiry(text, text, text, text, text, text, text, jsonb);
...
revoke all on function public.submit_customer_inquiry(text, text, text, text, text, text, text, jsonb) from public;
grant execute on function public.submit_customer_inquiry(text, text, text, text, text, text, text, jsonb) to anon, authenticated;
